<script lang="ts">
	...
	function updateQuery() {
		...
		if (
			prevUrl.searchParams.get('start_date') !== url.searchParams.get('start_date') ||
...
<main class="mx-auto w-full max-w-7xl space-y-4 p-4 sm:p-6">
			...
			<label class="space-y-1 text-sm">
				...
				<div class="flex gap-2">
					<input
						...
						class="rounded-md border border-border bg-background px-3 py-2 text-sm shadow-sm"
					/>
					<Button
						variant="outline"
						disabled={!selectedEndDate}
						onclick={() => (selectedEndDate = '')}
					>
						Clear
					</Button>
				</div>
	...
	<div class="flex items-center gap-2 justify-end">
		...
		<span class="text-sm text-muted-foreground"
			>of {totalPages} page{totalPages === 1 ? '' : 's'}</span
		>
		<Button
			variant="outline"
			onclick={() => {
				offset = Math.min(offset + 1, totalPages);
				updateQuery();
			}}
		>
			Next
