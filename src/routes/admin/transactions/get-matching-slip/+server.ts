export const GET = async ({ request, platform, locals }) => {
	...
	const found = await platform?.env.AccountingDatabase.prepare(
		...
		.bind(transactionId, bank)
		.first();

	if (!found) {
		return new Response(JSON.stringify({ error: 'No matching slip found', success: false }), {
			status: 200
		});
	}

	return new Response(JSON.stringify({ success: true, data: found.slipok_response }), {
		status: 200
	});
};
