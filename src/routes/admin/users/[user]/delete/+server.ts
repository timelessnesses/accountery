...
export const POST = async ({ params, platform, locals }) => {
    ...
    try {
        await env?.AccountingDatabase.prepare('DELETE FROM users WHERE email = ?').bind(userEmail).run();
        await env?.AccountingDatabase.prepare('INSERT INTO logs (email, action, timestamp) VALUES (?, ?, ?)').bind(
            locals.user.email,
