<script lang="ts">
    ...
    const info = linkedUserAccountWithInfo[(data.user?.email.split("@")[0]) as unknown as keyof typeof linkedUserAccountWithInfo] ?? {};
    const totalPaid = $derived([...data.allTransactionsFromUser].filter((tx) => tx.approved === 'approved').reduce((sum, tx) => sum + tx.amount, 0));
	const totalPending = $derived([...data.allTransactionsFromUser].filter((tx) => tx.approved === 'pending').reduce((sum, tx) => sum + tx.amount, 0));
	const totalOwed = $derived(data.allObligations.reduce((sum, ob) => sum + ob.amount, 0));
</script>

<h2 class="text-center">User {info.name} ({data.user.email}) <span>Paid: {totalPaid} + {totalPending} = {totalPaid + totalPending}</span> Owned: {totalOwed} Net: {totalPaid - totalOwed}</h2>
<div class="mt-6 grid gap-6 lg:grid-cols-[280px_1fr_360px]">

    <!-- Transaction history -->
    <div class="min-h-0">
        <TransactionLog
            transactions={data.allTransactionsFromUser}
        />
    </div>
    ...
    <div class="rounded-xl border border-border bg-card p-4">
        ...
        {#if selectedWeek}
            ...
            <div class="mt-3 flex items-center gap-2">
                <span
                    class="h-2 w-2 rounded-full {meta.dot}"
                ></span>
